...
export default async function handler(req, res) {
  ...
  try {
    ...
    const youthRes = await fetch(
      `${supabaseUrl}/rest/v1/youth?champion_id=eq.${champion_id}&status=not.in.(removed,completed)&end_of_challenge_completed_at=is.null&select=id,first_name,last_name,email,phone,status,stage_entered_at,accepted_at,first_drop_url,application_id,full_send_url,end_of_challenge_completed_at,token_expires_at`,
      { headers }
    );
